...
function direction_selected(select_direction) # Runs when clicking on the arrows
...
# ╔═╡ 28c4e24c-94a5-4d98-9bcd-34de56c10fa4
function menu()
    println(" "^28*" ┌──────────────────┐    ")
    println(" "^28*" │    Welkom dans   │    ")
    println(" "^28*" │   Paper Soccer ! │    ")	
    println(" "^28*" └──────────────────┘    ")
    println(" "^28*" 1. Play                ")
	println(" "^28*" 2. Rules of the game   ")
end
...
begin
		...
		h = @htl(
			...
			<style>
				...
				input[type="radio"]:hover + svg{
					fill: rgb(100, 204, 143);
				}
			
				input[type="radio"]:checked + svg,
				input[type="radio"]:focus + svg {
					fill: rgb(0, 204, 79); 		   // Color of the arrows (lightgrey)
				}
			</style>
			""")
		show(io, m, h) # To 
	...
	md"""Code that displays the svg created with the draw_arrows function"""
end
...
$(@bind name_player_2 TextField(default="Bossey"))
"""
...
begin
	...
	resetgame = GameState((0,0), initialize_grid(), initialize_vectors(), initialize_datadraw(),NAME_1, COLOR_1)
...
function play_game(direction)
	...
	if values(gamestate.vectors[coords, newcoords]) == 0 # Draw only if direction = 0
...
# ╔═╡ f1062dba-e10a-4586-8098-eca2c62f9b18
begin
