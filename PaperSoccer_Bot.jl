...
function new_coords(coords, direction)
	...
	return newcoords # Final position of the ball
end
...
When the temperature is low, the probabilities tend to go to the most weighted direction (heading towards the player's goal). And when the temperature is high, all the directions have nearly equal probability (resulting in the bot playing randomly).
"""
...
md"""
## HTML → Boutons
Thanks to the functions of 'de Villenfagne de Vogelsanck' who helped with this part.
"""
...
begin
	...
	function Base.show(io::IO, m::MIME"text/html", radio::Radio)
		...
		"""<script>
    		function resetRadio(radioButtonId) {
            ...
            @htl("""
                    $(draw_arrows(eval(Meta.parse(o.first)),"begin"))
                        ...
                        <input $((
									...
									onclick="resetRadio('$(groupname * o.first)')"
                                ))>
                        $(draw_arrows(eval(Meta.parse(o.first)),"svg"))
                        </label>
        ...
        <script>
			...
			let val = selected_radio?.value 	// Set value of the button to "val"
			...
			<style>
				form {
				text-align: center; 		// Set the svg (arrows) in the center
				}
				input[type="radio"] {
					position: absolute;
					opacity: 0; 	// Make the checkbox invisible behind the arrows
				}
				
				input[type="radio"] + svg {
				  fill: rgb(242, 242, 242);
					height: 2rem; 						// Size of the icons
					width: 2rem;begin 					// Size of the icons
					margin: 0rem;
					transition: all 0.1s;
				}
				...
				input[type="radio"]:hover + svg{
					fill: rgb(100, 204, 143); 		// 2nd color of the arrows (green)
				}
			
				input[type="radio"]:checked + svg,
				input[type="radio"]:focus + svg {
					fill: rgb(0, 204, 79); 		   // 1st color of the arrows (grey)
				}
			</style>
			""")
		show(io, m, h)
	...
	md"""Code that make the draw_arrows function works"""
end
...
function update_grid(grid, coords)
			if haskey(grid, coords) # To check if the coordinate is in the grid
				...
				elseif grid[coords] == 1
					println("Please replay") # CHANGE TO >REPLAY< ?
				end
			else 
				error("You have already scored, please reset the game to play again")
			end
...
function initialize_grid()
	...
	for row in -(ROWS/2):(ROWS/2) # Set the grid
...
$(@bind name_player_1 TextField(default="Habets"))
"""
...
function loosing_conditions()
	...
	dirup = [3,4,5] # Allowed directions from upper line
	dirdown = [0,1,7] # Allowed directions from lower line
	dirleft = [1,2,3] # Allowed directions from left line
	dirright = [5,6,7] # Allowed directions from right line
	test = 0
	coords = gamestate.coords
	if coords[2] == (ROWS-2)/2 # To try the directions from upper line
		for dir in dirup 
	...
	elseif coords[2] == -(ROWS-2)/2 # To try the directions from lower line
		for dir in dirdown
			newcoords = new_coords(coords,dir)
			if values(gamestate.vectors[coords,newcoords]) == 0
				test += 1
	...
	elseif coords[1] == -(COLS-2)/2 # To try the directions from left line
		for dir in dirleft 
			newcoords = new_coords(coords,dir)
			if values(gamestate.vectors[coords,newcoords]) == 0
				test += 1
	...
	elseif coords[1] == (COLS-2)/2 # To try the directions from right line
		for dir in dirright
			newcoords = new_coords(coords,dir)
			if values(gamestate.vectors[coords,newcoords]) == 0
				test += 1
	...
	else # To try from any other coordinate in the grid
		for dir in directions
			newcoords = new_coords(coords, dir)
			if values(gamestate.vectors[coords,newcoords]) == 0
				test += 1
...
function draw_datadraw(t)
	for key in sort(collect(keys(gamestate.datadraw))) # To get the right order
		eachdata = gamestate.datadraw[key] # To get the color and direction of each
		datacolor = eachdata[1] # To get the color
		datadir = eachdata[2]
		add_line(t, datacolor, datadir) # To draw the line(s) on the Turtle
	end
end
...
begin
	...
	Click here to go in the same direction as the previous one $(@bind go Button("Go!")) (in dev.)
	"""
end
...
# ╔═╡ b56360f3-d7e6-4be1-9616-0bce484b7081
begin
	select_direction # To make it run everytime the game is updated
	go

	if gamestate.coords == (0, ROWS/2) 
		println("$NAME_1 has won")
	elseif gamestate.coords == (0, -ROWS/2)
		println("$NAME_2 has won")
	else
		println("It is $(gamestate.player)'s turn")
	end
end
