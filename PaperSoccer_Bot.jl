...
begin
	...
	using Random							# For the HTML/Javascript code
...
More informations on the notebook called _PaperSoccer-BotExplained.jl_ that you can find on my GitHub.
"""
...
"""
probabilities(directions)

This function takes an array of directions as input as returns an array with the associated probabilities. Those probabilities changes in function of the direction (weight) and in function of the temperature set by the user.
"""
...
This function returns an array with all the possible directions from the actual position.
"""
function directions()
...
"""
nogo_directions(direction)

This function checks if the bot is blocked after moving in the previously selected direction or if it will go to some forbidden places (corners, in front of its goal if he cannot replay...). The function return 'false' if a condition is not fullfilled and 'true' is all the conditions are fullfilled.
"""
...
This function takes a direction (Int64) as input and return a new direction if the bot is at some precise locations. If the conditions are not fullfilled, the previous direction will be returned.
"""
...
"""
check_blocked(direction)

This function takes a direction (Int64) as input and checks if the bot is going to be blocked after 3 forced moves with the first one being the choosen direction.
...
This function does not take any input and returns a direction selected by a set of functions.
"""
function run_bot()
...
This function calls the play_game function with the direction choosen by the player or with the direction choosen by the bot.
"""
function play()
